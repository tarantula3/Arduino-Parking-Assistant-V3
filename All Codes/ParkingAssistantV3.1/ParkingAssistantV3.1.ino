...
const char* SmartURL     = "http://192.168.0.7/Arduino/Garage/UpdateCarDistance.php";
const char* UpdateStatus = "http://192.168.0.7/Arduino/Garage/UpdateStatus.php";
...
void setup() {
  ...
  // Send the initial wakeup message to the server
  SendIamAlive();             
...
void loop() {
  ...
  if (((distance+5) >= TempDistance) && ((distance-5) <= TempDistance)) {
    if(counter >= 20) { 
      ...
      if ( flashRainbow ) { SendDistance(); rainbow(2); flashRainbow = false; }; // Send the distance flash the rainbow and turn off
      turnThemAllOff();
    } else { counter++;   };
...
void SendIamAlive(){
  WiFiConnect( UpdateStatus, String("?Token=32432dewfd435zft56he5d") + String("&Sensor=Parkingv3") );
};

//// Function that sends Temperature and Humidity info to the server
void SendDistance() {
  WiFiConnect( SmartURL, String("?DISTANCE=") + distance + String("&Location=Garage") );
};
